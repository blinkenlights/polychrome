...
| Installation | Slot | Device UDP | Layout |
|--------------|------|------------|--------|
| `Octopus.Installation.Pixie` | `port: 1` | 1337 | 8×8 |
| `Octopus.Installation.Pixie2` | `port: 2` | 1338 | 8×8 |
| `Octopus.Installation.Woodstock` | `port: 2` | 1338 | 2×32 |

Run two processes with distinct installation modules. No fixed local bind ports are
needed — the OS assigns ephemeral ports and the firmware learns the reply address
from incoming pixel packets:
...
# Terminal 2 — Pixie2 (GPIO 3 / UDP 1338)
INSTALLATION_MODULE=Octopus.Installation.Pixie2 \
  PORT=4001 \
  mix phx.server
```

Firmware replies to whichever source port sent pixel data on that bus — it does not hardcode Octopus ports.
