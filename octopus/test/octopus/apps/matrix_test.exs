defmodule Octopus.Apps.MatrixTest do
  ...
  test "list_modes/0 includes matrix mode" do
    [mode] = matrix_list_modes()
    assert mode.id == "matrix:matrix"
    assert mode.builtin == true
  end

  test "mode_config/1 returns defaults" do
    assert matrix_mode_config("matrix:matrix") == %{speed: 1.0, density: 3, max_particles: 200}
    assert matrix_mode_config("matrix") == %{speed: 1.0, density: 3, max_particles: 200}
  ...
  test "get_config/1 returns tweakable fields" do
    ...
    assert %{speed: 2.0, density: 5, max_particles: 150} = matrix_get_config(state)
  end
  ...
  test "handle_config/2 applies speed, density, and max_particles live" do
    ...
    {:noreply, updated} =
      matrix_handle_config(%{speed: 1.5, density: 7, max_particles: 300}, state)
  ...
  test "now_playing_meta/1 summarizes density and max particles" do
    assert matrix_now_playing_meta(%{max_particles: 250, density: 6}) == [
             "250 particles max",
             "density 6"
           ]
  end
