...
REMOTE_HOST="gravity.crested-frog.ts.net"
REMOTE_USER="tim"
...
if [ ! -f "${ENV_FILE}" ]; then
    ...
    echo "Copy deploy/gravity/.env.example to deploy/gravity/.env and fill in values."
    exit 1
fi
...
rsync -av "${SCRIPT_DIR}/docker-compose.yml" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/deploy/gravity/"
rsync -av "${SCRIPT_DIR}/Caddyfile" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/deploy/gravity/"
rsync -av "${ENV_FILE}" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/deploy/gravity/.env"

# Deploy on remote
echo "Deploying on ${REMOTE_HOST}..."
ssh "${REMOTE_USER}@${REMOTE_HOST}" "
    set -e
    cd ${REMOTE_DIR}/deploy/gravity
...
echo "Done. Check logs with: ssh ${REMOTE_USER}@${REMOTE_HOST} 'cd ${REMOTE_DIR}/deploy/gravity && docker compose logs -f'"
